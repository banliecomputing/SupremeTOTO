...
(window as any).appConfig = {
  apiKey: Object.is((window as any).DEFAULT_API_KEY, undefined)
    ? ""
    : (window as any).DEFAULT_API_KEY,
};
(window as any).pools = [];
...
(window as any).updateGeminiStatusIndicator = (
  ...
  if (status === "ready") {
    textEl.className =
      "text-[8px] text-emerald-400 font-extrabold tracking-wider leading-none truncate";
    ...
    textEl.className =
      "text-[8px] text-purple-400 font-extrabold tracking-wider leading-none truncate animate-pulse";
    textEl.innerHTML = `AI: PROSES`;
    iconEl.className =
      "ph-fill ph-circle text-[6px] text-purple-500 animate-pulse shrink-0";
    textEl.parentElement?.setAttribute(
      "title",
      "Gemini AI: Sedang memproses analisis.",
    );
  } else if (status === "failed") {
    textEl.className =
      "text-[8px] text-red-500 font-extrabold tracking-wider leading-none truncate";
    textEl.innerHTML = `AI: GAGAL`;
    iconEl.className =
      "ph-fill ph-circle text-[6px] text-red-500 shrink-0 animate-pulse";
    textEl.parentElement?.setAttribute(
      "title",
      message || "Gemini AI: Gagal. Klik Mulai untuk detail.",
    );
  }
};
...
(window as any).robustFetch = async (url: string): Promise<string> => {
  ...
  for (const getProxyUrl of proxies) {
    try {
      ...
      if (!response.ok) {
        console.warn(
          `[robustFetch] Proxy failed with status ${response.status}`,
        );
        continue;
      }
  ...
  throw (
    lastError ||
    new Error(
      `Gagal memuat data dari ${url}. Semua jalur CORS proxy sedang sibuk.`,
    )
  );
};
...
(window as any).runLayoutCorrection = (container: HTMLElement) => {
  ...
  const shioTextElement = container.querySelector(
    "#shio-name-text",
  ) as HTMLElement | null;
  const shioDisplay = container.querySelector(
    "#shio-emoji-display",
  ) as HTMLElement | null;
  if (shioTextElement && shioDisplay) {
    let shioText = (
      shioTextElement.innerText ||
      shioTextElement.textContent ||
      ""
    ).toUpperCase();
...
(window as any).syairTemplate = {
  ...
  accentColor: "#b45309",
  glow: true,
  footer: "Utamakan Prediksi Sendiri",
      ...
      requestFullScreen
        ...
        .catch((err: any) => {
          (window as any).showToast(
            "Mode Fullscreen tidak diizinkan di jendela pratinjau ini.",
            true,
          );
        });
    }
...
(window as any).closeTablePreview = () => {
  ...
  if (modal) {
    modal.classList.replace("opacity-100", "opacity-0");
    document
      .getElementById("table-preview-content")!
      .classList.replace("scale-100", "scale-95");
    setTimeout(() => modal.classList.add("hidden"), 300);
  }
};
...
(window as any).showAIScenario = () => {
  ...
  const poolSel = document.getElementById(
    "ai-pool-selector",
  ) as HTMLSelectElement;
  const poolName =
    poolSel && poolSel.options[poolSel.selectedIndex]?.value !== ""
      ? poolSel.options[poolSel.selectedIndex].text
      : '<span class="text-red-400">Belum dipilih (Pilih di atas)</span>';

  const promptInput = (
    document.getElementById("analisis-prompt") as HTMLTextAreaElement
  ).value;
  const promptInfo =
    promptInput.trim() !== ""
      ? promptInput
      : '<span class="text-slate-500 italic">Tidak ada instruksi spesifik (AI hanya akan membaca System Prompt)</span>';

  const activeSources = (window as any).extraSources.filter(
    (s: any) => s.active,
  );
  let sourcesInfo =
    '<ul class="list-disc pl-5 text-slate-300 space-y-1 text-xs md:text-sm mt-2">';
  ...
  if (
    ...
    sourcesInfo =
      `<div class="mb-2 p-3 bg-blue-500/10 border border-blue-500/20 rounded text-amber-300 text-xs italic shadow-inner"><b>Rule Global Sandingan:</b> ${(window as any).sandinganConfig.globalPrompt}</div>` +
      sourcesInfo;
  }
  ...
  if (activeSources.length > 0) {
    activeSources.forEach((s: any) => {
      let promptText = s.prompt
        ? `<br><span class="text-amber-400 italic font-normal inline-block mt-2 bg-amber-500/10 px-2 py-1 rounded border border-amber-500/20"><i class="ph-fill ph-robot text-xs"></i> ${s.prompt}</span>`
        : "";
      sourcesInfo += `<li class="mb-2"><span class="font-bold text-emerald-400">${s.title}</span> (Range: ${s.range || "Semua"})${promptText}</li>`;
    });
  } else {
    sourcesInfo +=
      '<li><span class="text-slate-500 italic">Tidak ada data sandingan tambahan yang aktif.</span></li>';
  }
  sourcesInfo += "</ul>";

  let globalRulesInfo =
    '<ul class="list-decimal pl-5 text-slate-300 space-y-2 text-xs md:text-sm mt-2">';
  if ((window as any).globalRules.length > 0) {
    (window as any).globalRules.forEach(
      (r: any) =>
        (globalRulesInfo += `<li class="border-b border-slate-800 pb-2">${r.text}</li>`),
    );
  } else {
    globalRulesInfo +=
      '<li><span class="text-slate-500 italic">Tidak ada rule global / system prompt aktif.</span></li>';
  }
  globalRulesInfo += "</ul>";
  ...
  setTimeout(() => {
    ...
    document
      .getElementById("ai-scenario-content")!
      .classList.replace("scale-95", "scale-100");
  }, 10);
};
...
(window as any).previewRawPrompt = async () => {
  ...
  if (!sel || !sel.value)
    return (window as any).showToast("Pilih pasaran target dulu!", true);
...
(window as any).switchTab = (tabName: string) => {
  document
    ...
    .forEach((p) => p.classList.replace("page-active", "page-hidden"));
  document
    .querySelectorAll(".nav-btn-drawer")
    .forEach((b) =>
      b.classList.remove("bg-emerald-500/10", "text-emerald-400"),
    );
  ...
  if (tabName === "ai") {
    const aiPoolSel = document.getElementById(
      "ai-pool-selector",
    ) as HTMLSelectElement;
  ...
  const d = document.getElementById("main-nav-drawer");
  if (d && d.classList.contains("translate-x-0"))
    (window as any).toggleMainNav();
};
...
(window as any).openEditModal = (
  ...
  if (srcEl && modalBody && modal && content) {
    ...
    document.getElementById("edit-modal-title")!.innerHTML =
      `<i class="ph-bold ph-pencil"></i> ${titleText}`;
    modal.classList.remove("hidden");
...
(window as any).closeEditFormModal = () => {
  if (
    (window as any).editingPasaranId ||
    document.getElementById("container-form-pasaran")?.parentElement?.id ===
      "edit-modal-body"
  ) {
    (window as any).cancelEditPasaran();
  } else if (
    (window as any).editingMimpiId ||
    document.getElementById("container-form-mimpi")?.parentElement?.id ===
      "edit-modal-body"
  ) {
    (window as any).cancelEditMimpi();
  } else if (
    (window as any).editingPromptId ||
    document.getElementById("container-form-prompt")?.parentElement?.id ===
      "edit-modal-body"
  ) {
    (window as any).cancelEditPrompt();
  } else if (
    (window as any).editingGlobalRuleId ||
    document.getElementById("container-form-global")?.parentElement?.id ===
      "edit-modal-body"
  ) {
    (window as any).cancelEditGlobalRule();
  } else if (
    (window as any).editingSourceId ||
    document.getElementById("form-sandingan")?.parentElement?.id ===
      "edit-modal-body"
  ) {
...
(window as any).openSubAdmin = (menuId: string) => {
  ...
  const titleMap: any = {
    "sub-admin-pasaran": "KELOLA PASARAN",
    "sub-admin-mimpi": "BUKU MIMPI",
    "sub-admin-prompt": "DATABASE POLA",
    "sub-admin-global": "SYSTEM PROMPT",
    "sub-admin-settings": "PENGATURAN SISTEM",
    "sub-admin-spreadsheet": "PUSAT DATA & IMPOR",
...
(window as any).closeSubAdmin = () => {
  ...
  if (typeof (window as any).cancelEditPasaran === "function")
    (window as any).cancelEditPasaran();
  if (typeof (window as any).cancelSandingan === "function")
    (window as any).cancelSandingan();
};
...
(window as any).loadTabsForURLValue = async (url: string, nameId: string) => {
  if (!url)
    return (window as any).showToast(
      "Silakan isi URL Spreadsheet terlebih dahulu!",
      true,
    );
  ...
  try {
    ...
    const res = await (window as any).robustFetch(
    ...
    if (names.length === 0) {
      ...
      items.forEach((a) => {
        const n = (a as HTMLElement).innerText.trim();
        if (n) names.push(n);
      });
    }
    ...
    if (names.length > 0) {
      ...
      (window as any).showToast(
        "Nama Tab gagal dilacak! Pastikan URL Publik.",
        true,
      );
    }
  } catch (e: any) {
...
(window as any).showTabSelector = (inputId: string, tabs: string[]) => {
  ...
  list.className =
    "absolute z-50 bg-slate-800 border border-emerald-500 rounded-lg shadow-[0_10px_25px_rgba(0,0,0,0.8)] max-h-48 overflow-y-auto w-full mt-1 top-full left-0";

  tabs.forEach((t) => {
    const item = document.createElement("div");
    item.className =
      "p-3 text-xs md:text-sm text-white hover:bg-emerald-600 cursor-pointer border-b border-slate-700/50 last:border-0 font-bold flex items-center gap-2";
  ...
  if (inp.parentElement!.style.position !== "relative") {
...
(window as any).indexToA1 = (r: number, c: number) => {
  ...
  let tempC = c;
  while (tempC >= 0) {
    colStr = String.fromCharCode((tempC % 26) + 65) + colStr;
...
const fetchSpreadsheetTab = async (
  ...
  try {
    ...
    if (
      csvProx.trim().toLowerCase().startsWith("<!doctype html>") ||
      csvProx.trim().toLowerCase().startsWith("<html")
    ) {
...
(window as any).openVisualSelector = async (
  urlId: string,
  sheetNameId: string,
  targetRangeId: string,
) => {
  const url = (document.getElementById(urlId) as HTMLInputElement).value;
  const sheetName = (document.getElementById(sheetNameId) as HTMLInputElement)
    .value;
  const sheetId = getSpreadsheetId(url);

  if (!sheetId || !sheetName)
    return (window as any).showToast(
      "Harap isi URL & Nama Sheet dulu untuk memuat tabel Visual!",
      true,
    );
  ...
  setTimeout(() => {
    modal!.classList.replace("opacity-0", "opacity-100");
    document
      .getElementById("range-picker-content")!
      .classList.replace("scale-95", "scale-100");
  }, 10);
  ...
  try {
    ...
    let tableHtml =
      '<table class="w-full text-left border-collapse text-[10px] md:text-sm whitespace-nowrap bg-slate-900" id="visual-selector-table">';
    tableHtml +=
      '<thead class="bg-slate-800 text-slate-300 sticky top-0 z-20 shadow-md"><tr><th class="p-2 md:p-3 border border-slate-700 bg-slate-950 sticky left-0 z-30 w-8 md:w-12 text-center">#</th>';
    for (let c = 0; c < maxCols; c++) {
      tableHtml += `<th class="p-2 md:p-3 border border-slate-700 text-center min-w-[80px] md:min-w-[120px]">${(window as any).indexToA1(0, c).replace(/[0-9]/g, "")}</th>`;
    }
...
(window as any).updateVisualHighlight = () => {
  ...
  document.querySelectorAll(".range-cell").forEach((td) => {
    td.classList.remove("range-cell-selected", "range-cell-in-range");
  });
  ...
  if (sel.start) {
    ...
    const startTd = document.querySelector(
      `.range-cell[data-r="${sel.start.r}"][data-c="${sel.start.c}"]`,
    );
      ...
      for (let r = minR; r <= maxR; r++) {
        for (let c = minC; c <= maxC; c++) {
          const td = document.querySelector(
            `.range-cell[data-r="${r}"][data-c="${c}"]`,
          );
          if (td) {
            if (
              (r === sel.start.r && c === sel.start.c) ||
              (r === sel.end.r && c === sel.end.c)
            ) {
              td.classList.add("range-cell-selected");
            } else {
              td.classList.add("range-cell-in-range");
            }
          }
        }
      }
...
(window as any).resetVisualSelection = () => {
  (window as any).rangeSelection.start = null;
  (window as any).rangeSelection.end = null;
  (window as any).updateVisualHighlight();
};
...
(window as any).confirmVisualSelection = () => {
  ...
  if (sel.start && target && resEl) {
    target.value = resEl.innerText;
    (window as any).closeVisualSelector();
    (window as any).showToast("Rentang berhasil dipilih!");
    if ("createEvent" in document) {
      var evt = document.createEvent("HTMLEvents");
      evt.initEvent("change", false, true);
    ...
    (window as any).showToast("Klik minimal 1 sel di tabel!", true);
  }
};
...
(window as any).renderLiveDrawsEditor = () => {
  ...
  if ((window as any).currentLiveDraws.length === 0) {
    container.innerHTML =
      '<p class="text-[10px] text-slate-500 text-center py-4 bg-slate-950 rounded-lg border border-slate-800 border-dashed">Belum ada putaran. Klik + Tambah Putaran.</p>';
    return;
  }
...
(window as any).addLiveDrawField = () => {
  (window as any).currentLiveDraws.push({
    name: `Putaran ${(window as any).currentLiveDraws.length + 1}`,
    urlLive: "",
    sheetNameLive: "",
    rangeLive: "",
  });
...
(window as any).removeLiveDraw = (idx: number) => {
  (window as any).currentLiveDraws.splice(idx, 1);
  (window as any).renderLiveDrawsEditor();
};
(window as any).updateLiveDraw = (idx: number, field: string, value: any) => {
  (window as any).currentLiveDraws[idx][field] = value;
};

(window as any).renderDropdowns = () => {
  const els = [
    document.getElementById("pool-selector"),
    document.getElementById("ai-pool-selector"),
  ];
  els.forEach((el) => {
    if (!el) return;
...
(window as any).renderAdminLists = () => {
  ...
  if (pList) {
    pList.innerHTML =
      (window as any).pools.length === 0
        ? '<p class="text-sm text-slate-500 text-center py-4">Belum ada pasaran</p>'
        : "";
    (window as any).pools.forEach((p: any) => {
      let mappingTxt = p.mapCols
        ? `<br><span class="text-amber-500">Map: Kolom(${p.mapCols}) ${p.autoFill ? "[Auto-Fill Aktif]" : ""}</span>`
        : "";
  ...
  if (eList) {
    eList.innerHTML =
      (window as any).extraSources.length === 0
        ? '<p class="text-xs md:text-sm text-slate-500 p-4 bg-slate-900 rounded border border-slate-800 border-dashed text-center">Belum ada Data Sandingan.</p>'
        : "";
    (window as any).extraSources.forEach((src: any) => {
      ...
      if (
        !src.poolIds ||
        src.poolIds.length === 0 ||
        src.poolIds.includes("all")
      ) {
        ...
        const names = src.poolIds
          .map((pId: string) => {
            const pObj = (window as any).pools.find((x: any) => x.id === pId);
            return pObj ? pObj.name : pId;
          })
          .join(", ");
        poolBadges = `<span class="text-[9px] bg-blue-500/15 text-blue-400 border border-blue-500/20 font-black px-2 py-0.5 rounded uppercase font-sans flex items-center gap-1 max-w-full justify-start mt-1.5"><i class="ph-bold ph-globe-hemisphere-west text-xs"></i> Pasaran: ${names}</span>`;
      }

      eList.innerHTML += `
                <div class="bg-slate-800 p-3 md:p-4 rounded-lg border border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center shadow-sm gap-2">
                    <div class="flex-1 min-w-0 pr-3 cursor-pointer" onclick="toggleExtraSource('${src.id}', ${!src.active})">
                        ...
                        <div class="pl-9">${poolBadges}</div>
                    </div>
                    <div class="flex gap-1 shrink-0 ml-9 sm:ml-0">
  ...
  if (prList)
    prList.innerHTML =
      (window as any).aiPrompts.length === 0
        ? '<p class="text-sm text-slate-500 text-center py-4">Belum ada pola prompt</p>'
        : "";
  ...
  if (gList) {
    gList.innerHTML =
      (window as any).globalRules.length === 0
        ? '<p class="text-sm text-slate-500 text-center py-6 bg-slate-900 rounded-lg">Otak AI Kosong. Harap Reset atau Injeksi Aturan.</p>'
        : "";
    (window as any).globalRules.forEach((g: any) => {
      let poolBadges = "";
      if (!g.poolIds || g.poolIds.length === 0 || g.poolIds.includes("all")) {
        poolBadges = `<span class="text-[9px] bg-slate-800 text-slate-400 border border-slate-700 font-black px-2 py-0.5 rounded uppercase font-sans flex items-center gap-1 shrink-0"><i class="ph-bold ph-globe text-xs"></i> Semua Pasaran</span>`;
      } else {
        const names = g.poolIds
          .map((pId: string) => {
            const pObj = (window as any).pools.find((x: any) => x.id === pId);
            return pObj ? pObj.name : pId;
          })
          .join(", ");
        poolBadges = `<span class="text-[9px] bg-amber-500/15 text-amber-400 border border-amber-500/20 font-black px-2 py-0.5 rounded uppercase font-sans flex items-center gap-1 max-w-full justify-start mt-1.5"><i class="ph-bold ph-globe-hemisphere-west text-xs"></i> Pasaran: ${names}</span>`;
      }
                ...
                <div class="bg-slate-900 p-5 rounded-xl border border-slate-750 flex flex-col gap-3 shadow-md">
                    <div class="flex justify-between items-start gap-4">
                        <div class="flex flex-col">
                            <span class="text-[10px] font-bold text-amber-500 uppercase tracking-wider bg-amber-500/10 px-2.5 py-1 rounded border border-amber-500/20 flex items-center gap-1.5 w-max">
...
(window as any).populateAiPromptSelector = () => {
  ...
  aiSel.innerHTML =
    '<option value="">-- Tulis Instruksi Manual Bebas (Baca System Prompt) --</option>';
  ...
  if (pathPrompts.length > 0) {
    const allPromptsText = pathPrompts
      .map((p: any) => `[POLA: ${p.title}]\n${p.text}`)
      .join("\n\n");
...
(window as any).updateAIInfoPanel = () => {
  ...
  if (aiPoolSel) {
    activePoolId = aiPoolSel.value;
  }

  const activeSources = ((window as any).extraSources || []).filter(
    (s: any) => {
      if (!s.active) return false;
      if (!s.poolIds || s.poolIds.length === 0 || s.poolIds.includes("all"))
        return true;
      return activePoolId && s.poolIds.includes(activePoolId);
    },
  );

  if (activeSources.length > 0 && activePoolId) {
...
(window as any).renderBukuMimpi = () => {
  ...
  list.innerHTML = "";
  const filtered = (window as any).dataBukuMimpi.filter(
    (i: any) =>
      i.type === (window as any).currentMimpiType &&
      (i.no.includes(query) || i.desc.toLowerCase().includes(query)),
  );
...
(window as any).switchMimpiType = (type: string) => {
  ...
  document.querySelectorAll('[id^="tab-mimpi-"]').forEach((b) => {
    b.classList.replace("bg-emerald-500", "bg-slate-900");
    b.classList.replace("text-slate-900", "text-slate-400");
  });
...
(window as any).updateSyncTimeUI = (isAI: boolean, poolId: string) => {
  ...
  const lastSyncTime =
    localStorage.getItem(`last_sync_${poolId}`) || "Belum disinkronkan";
...
(window as any).fetchTableData = async (
  isAI = false,
  ...
  const silentPoolSelector = document.getElementById(
    "pool-selector",
  ) as HTMLSelectElement;
  const poolId =
    isSilentPreview && silentPoolSelector
      ? silentPoolSelector.value
      : poolSelector
        ? poolSelector.value
        : null;
  const container = isSilentPreview
    ? null
    : document.getElementById(isAI ? "ai-data-container" : "data-container");
  ...
  if (isAI) {
    (window as any).currentAnalyzedPoolName = targetPool.name;
    (window as any).populateAiPromptSelector();
    (window as any).updateAIInfoPanel();
  }
  ...
  if (container)
    container.innerHTML = `<div class="flex flex-col items-center justify-center h-full"><i class="ph ph-spinner-gap animate-spin text-4xl text-emerald-500 mb-3"></i><p class="text-sm font-bold text-emerald-400">Merakit Data...</p></div>`;
  ...
  try {
    ...
    if (sheetId && targetPool.sheetName) {
      const csvData = await fetchSpreadsheetTab(
        sheetId,
        targetPool.sheetName,
        "",
        targetPool.rangeHistory,
      );
      ...
      const htmlContent = await (window as any).robustFetch(
        targetPool.urlHistory,
      );
      ...
      const docParser = new DOMParser().parseFromString(
        htmlContent,
        "text/html",
      );
    ...
    if (rawMatrix.length > 0) {
      if (targetPool.mapCols) {
        ...
        const globalIndices = targetPool.mapCols
          .split(",")
          .map((s: string) => (window as any).colToIndex(s))
          .filter((i: number) => i >= 0);
        const headers = (targetPool.mapHeaders || "")
          .split(",")
          .map((s: string) => s.trim());

        const newHeaders: string[] = ["Tanggal / Hari"];
        globalIndices.forEach((globalIdx: number, i: number) =>
          newHeaders.push(
            headers[i] || `KOLOM ${String.fromCharCode(65 + globalIdx)}`,
          ),
        );
        if (newHeaders.length > 1)
          (window as any).currentTableData.push(newHeaders);
    ...
    if (!isSilentPreview && container) {
      if ((window as any).currentTableData.length > 0) {
        ...
        wrapper.className =
          "w-full overflow-x-auto rounded-xl border border-slate-700/50 shadow-xl bg-slate-900 h-full custom-scrollbar";
        const tbl = document.createElement("table");
        tbl.className =
          "min-w-full text-sm md:text-base text-slate-300 border-collapse whitespace-nowrap";

        (window as any).currentTableData.forEach(
          (rowArray: any, index: number) => {
...
(window as any).addPasaran = async (e: any) => {
  e.preventDefault();
  if (!(window as any).cloudUserId)
    return (window as any).showToast("Cloud DB belum siap", true);
  ...
  const name = (document.getElementById("input-name") as HTMLInputElement)
    .value;
  const imageUrl = (
    document.getElementById("input-image-url") as HTMLInputElement
  ).value;
  const urlHistory = (
    document.getElementById("input-url-history") as HTMLInputElement
  ).value;
  const sheetName = (
    document.getElementById("input-pasaran-sheetname") as HTMLInputElement
  ).value;
  const rangeHistory = (
    document.getElementById("input-pasaran-rangehistory") as HTMLInputElement
  ).value;
  const mapCols = (
    document.getElementById("input-pasaran-map-cols") as HTMLInputElement
  ).value;
  const mapHeaders = (
    document.getElementById("input-pasaran-map-headers") as HTMLInputElement
  ).value;
  const autoFill = (
    document.getElementById("input-pasaran-autofill") as HTMLInputElement
  ).checked;
  const visible = (document.getElementById("input-visible") as HTMLInputElement)
    .checked;
...
(window as any).editPasaran = (id: string) => {
  ...
  (document.getElementById("input-image-url") as HTMLInputElement).value =
    p.imageUrl || "";
  (document.getElementById("input-url-history") as HTMLInputElement).value =
    p.urlHistory || "";
  (
    document.getElementById("input-pasaran-sheetname") as HTMLInputElement
  ).value = p.sheetName || "";
  ...
  (document.getElementById("input-visible") as HTMLInputElement).checked =
    p.visible !== false;

  (window as any).currentLiveDraws = p.liveDraws
    ? JSON.parse(JSON.stringify(p.liveDraws))
    : [];
  (window as any).renderLiveDrawsEditor();

  document.getElementById("btn-cancel-pasaran")?.classList.remove("hidden");
  (
    document.getElementById("btn-submit-pasaran") as HTMLButtonElement
  ).innerText = "UPDATE PASARAN CLOUD";
...
(window as any).cancelEditPasaran = () => {
  ...
  [
    ...
    "input-image-url",
    "input-url-history",
    "input-pasaran-sheetname",
    "input-pasaran-rangehistory",
    "input-pasaran-map-cols",
    "input-pasaran-map-headers",
  ].forEach((id) => {
  ...
  (
    document.getElementById("btn-submit-pasaran") as HTMLButtonElement
  ).innerText = "SIMPAN KE CLOUD";

  (window as any).closeEditModal(
    "container-form-pasaran",
    "original-slot-pasaran",
  );
};
...
(window as any).deletePasaran = (id: string) => {
  (window as any).showConfirm(
    "Yakin hapus pasaran ini secara permanen?",
    async () => {
...
(window as any).openFormSandingan = () => {
  ...
  if (form) {
    ...
    document.getElementById("title-form-sandingan")!.innerHTML =
      `<i class="ph-bold ph-plus-circle"></i> Tambah Koneksi Baru`;
    form.reset();
    (window as any).editingSourceId = null;
    (window as any).populateSandPoolsChecklist(["all"]);
  }
};
...
(window as any).saveGlobalSandinganPrompt = async (val: string) => {
  if (!(window as any).cloudUserId) return;
  await setDoc(
    getDocRef("settings", "sandingan_config"),
    { globalPrompt: val },
    { merge: true },
  );
};
...
(window as any).saveSandingan = async (e: any) => {
  ...
  const title = (document.getElementById("sand-title") as HTMLInputElement)
    .value;
  const url = (document.getElementById("sand-url") as HTMLInputElement).value;
  const sheetName = (document.getElementById("sand-sheet") as HTMLInputElement)
    .value;
  const range = (document.getElementById("sand-range") as HTMLInputElement)
    .value;
  const query = (document.getElementById("sand-query") as HTMLInputElement)
    .value;
  const prompt = (document.getElementById("sand-prompt") as HTMLInputElement)
    .value;
  const sheetId = getSpreadsheetId(url) || "";

  const cbs = document.getElementsByName(
    "sand-pool-ids",
  ) as NodeListOf<HTMLInputElement>;
  const allCb = document.getElementById("sand-pool-all") as HTMLInputElement;
  let selectedPools: string[] = [];
  if (allCb && allCb.checked) {
    selectedPools = ["all"];
  } else {
    cbs.forEach((cb) => {
      if (cb.checked) selectedPools.push(cb.value);
    });
  }

  if (url && url.trim().length > 0 && !sheetId) {
    (window as any).showToast(
      "URL Google Sheets disertakan tapi tidak valid. Harus mengandung /d/.../",
      true,
    );
  ...
  (window as any).showToast(
    (window as any).editingSourceId
      ? "Data Sandingan Diperbarui!"
      : "Data Sandingan Baru ditambahkan!",
  );
  (window as any).cancelSandingan();
...
(window as any).populateSandPoolsChecklist = (
  selectedPoolIds: string[] = [],
) => {
  const container = document.getElementById("sand-pools-container");
  if (!container) return;

  const pools = (window as any).pools || [];
  const isAllSelected =
    selectedPoolIds.includes("all") || selectedPoolIds.length === 0;
  ...
  pools.forEach((p: any) => {
    ...
    html += `
            <label class="flex items-center gap-2 bg-slate-950/60 p-2 rounded-xl border border-slate-800 hover:border-slate-700 cursor-pointer text-slate-300 hover:text-white transition-all">
                <input type="checkbox" name="sand-pool-ids" value="${p.id}" ${isChecked ? "checked" : ""} onchange="window.onIndividualSandPoolChange()" class="accent-blue-500 rounded bg-slate-900 border-slate-700">
  ...
  container.innerHTML =
    html ||
    '<p class="text-slate-500 italic text-[11px] py-2 col-span-2 text-center">Belum ada data pasaran.</p>';
};

(window as any).toggleAllSandPools = (el: HTMLInputElement) => {
...
(window as any).onIndividualSandPoolChange = () => {
  const allCheckbox = document.getElementById(
    "sand-pool-all",
  ) as HTMLInputElement;
  if (allCheckbox) {
...
(window as any).populateGlobalPoolsChecklist = (
  selectedPoolIds: string[] = [],
) => {
  const container = document.getElementById("global-pools-container");
  if (!container) return;
  ...
  pools.forEach((p: any) => {
    ...
    html += `
            <label class="flex items-center gap-2 bg-slate-950/60 p-2 rounded-xl border border-slate-800 hover:border-slate-700 cursor-pointer text-slate-300 hover:text-white transition-all">
                <input type="checkbox" name="global-pool-ids" value="${p.id}" ${isChecked ? "checked" : ""} onchange="window.onIndividualGlobalPoolChange()" class="accent-amber-500 rounded bg-slate-900 border-slate-700">
                <span class="truncate font-sans font-extrabold text-[10px] uppercase">${p.name}</span>
            </label>
        `;
  });
...
(window as any).toggleAllGlobalPools = (el: HTMLInputElement) => {
  const checkboxes = document.getElementsByName(
    "global-pool-ids",
  ) as NodeListOf<HTMLInputElement>;
  if (el.checked) {
...
(window as any).onIndividualGlobalPoolChange = () => {
  const allCheckbox = document.getElementById(
    "global-pool-all",
  ) as HTMLInputElement;
  if (allCheckbox) {
...
(window as any).populatePromptPoolsChecklist = (
  selectedPoolIds: string[] = [],
) => {
...
(window as any).addPrompt = async (e: any) => {
  ...
  const title = (
    document.getElementById("input-prompt-title") as HTMLInputElement
  ).value;
  const text = (
    document.getElementById("input-prompt-text") as HTMLTextAreaElement
  ).value;

  // Get selected pool IDs
  const poolCheckboxes = document.getElementsByName(
    "prompt-pool-ids",
  ) as NodeListOf<HTMLInputElement>;
  const allCheckbox = document.getElementById(
    "prompt-pool-all",
  ...
  try {
    await setDoc(
      doc(db, "prompts", id),
      { id, title, text, poolIds },
      { merge: true },
    );
    (window as any).showToast(
      (window as any).editingPromptId
        ? "Pola Prompt Diperbarui!"
        : "Pola Prompt Baru ditambahkan!",
    );
    (window as any).cancelEditPrompt();
  } catch (err: any) {
...
(window as any).editPrompt = (id: string) => {
  ...
  (document.getElementById("input-prompt-title") as HTMLInputElement).value =
    pr.title || "";
  (document.getElementById("input-prompt-text") as HTMLTextAreaElement).value =
    pr.text || "";
...
(window as any).cancelEditPrompt = () => {
  (window as any).editingPromptId = null;
  const titleIn = document.getElementById(
    "input-prompt-title",
  ) as HTMLInputElement;
  const textIn = document.getElementById(
    "input-prompt-text",
...
(window as any).deletePrompt = (id: string) => {
  (window as any).showConfirm(
    "Yakin menghapus pola prompt ini secara permanen?",
    async () => {
      if (!(window as any).cloudUserId) return;
      try {
...
(window as any).loadVpbPoolData = async () => {
  ...
  try {
    ...
    const activePoolSel = document.getElementById(
      "pool-selector",
    ) as HTMLSelectElement;
    ...
    row.slice(1).forEach((cellVal: string, cIdx: number) => {
      ...
      if (/^\d{4}$/.test(cellVal)) {
        ...
        for (let d = 0; d < 4; d++) {
          ...
          const isSelected = (window as any).vpbSelectedDigits.some(
            (x: any) => x.id === dKey,
          );
...
(window as any).toggleVpbCell = (
  ...
  if (existingIdx >= 0) {
    ...
    if (list.length >= 6) {
      return (window as any).showToast(
        "Maksimal 6 angka terpilih untuk formula",
        true,
      );
    }
    list.push({
...
(window as any).renderVpbSelectedBadges = () => {
  ...
  list.forEach((item: any) => {
    ...
    badge.className =
      "flex items-center gap-1.5 bg-slate-900 border border-slate-850 text-[10px] md:text-xs text-slate-300 pl-2 rounded-lg py-1 shadow-inner pr-1 hover:border-red-500/30 transition-colors group";

    let positionText =
      item.digitIdx === -1
        ? item.digitLabel
        : `${item.colHeader} (${item.digitLabel})`;
...
(window as any).generateVpbFormula = () => {
  ...
  if (list.length === 0) {
    return (window as any).showToast(
      "Pilih setidaknya satu angka dari tabel paito!",
      true,
    );
  }

  const relTypeEl = document.getElementById(
    "vpb-relation-type",
  ) as HTMLSelectElement;
  const targetVarEl = document.getElementById(
    "vpb-target-variable",
  ) as HTMLSelectElement;
  ...
  switch (relationType) {
    case "selisih":
      relationLabel = "Selisih (dikurangi)";
      break;
    case "jumlah":
      relationLabel = "Jumlah (ditambahkan)";
      break;
    case "mistik_lama":
      relationLabel = "Mistik Lama (ML)";
      break;
    case "mistik_baru":
      relationLabel = "Mistik Baru (MB)";
      break;
    case "index":
      relationLabel = "Index (IND)";
      break;
    case "tarikan_silang":
      relationLabel = "Tarikan Silang";
      break;
    case "shio_kaitan":
      relationLabel = "Korelasi Jalur Shio";
      break;
  }
  ...
  switch (targetVar) {
    case "BBFS":
      targetLabel = "meracik angka BBFS jitu";
      break;
    case "AM/AI":
      targetLabel = "menghasilkan Angka Main (AM) / Angka Ikut (AI) jitu";
      break;
    case "KEPALA/EKOR":
      targetLabel = "mencari draf Kepala dan Ekor akurat";
      break;
    case "JITU_2D":
      targetLabel = "merumuskan Angka Jitu 2D malam/hari ini";
      break;
    case "SHIO":
      targetLabel = "memetakan lintasan Shio mati dan Shio hidup";
      break;
  }
  ...
  if (sorted.length === 1) {
    ...
    const posStr =
      item.digitIdx === -1
        ? item.digitLabel
        : `${item.colHeader} bagian ${item.digitLabel}`;

    if (
      relationType === "mistik_lama" ||
      relationType === "mistik_baru" ||
      relationType === "index"
    ) {
    ...
    const posA =
      a.digitIdx === -1 ? a.digitLabel : `${a.colHeader} ${a.digitLabel}`;
    const posB =
      b.digitIdx === -1 ? b.digitLabel : `${b.colHeader} ${b.digitLabel}`;
...
(window as any).toggleVpbStudioLayout = () => {
  ...
  if ((window as any).vpbStudioLayoutMode === "split") {
    ...
    if (btnText)
      btnText.innerHTML =
        '<i class="ph-bold ph-monitor text-xs"></i> TAMPILAN: TABEL PENUH';
    ...
    if (btnText)
      btnText.innerHTML =
        '<i class="ph-bold ph-list-dashes text-xs"></i> TAMPILAN: FORM PENUH';
  } else {
    ...
    if (btnText)
      btnText.innerHTML =
        '<i class="ph-bold ph-layout text-xs"></i> TAMPILAN: SPLIT GRID';
  }
...
(window as any).openVpbStudioModal = () => {
  ...
  if (selector) {
    ...
    const mainSelector =
      (document.getElementById("vpb-pool-selector") as HTMLSelectElement) ||
      (document.getElementById("pool-selector") as HTMLSelectElement);
...
(window as any).renderVpbStudioTable = () => {
  ...
  const colors = [
    {
      ...
      active:
        "bg-purple-600 text-white border-purple-300 font-extrabold shadow-[0_0_20px_rgba(147,51,234,0.75)]",
    },
    {
      ...
      active:
        "bg-emerald-600 text-white border-emerald-300 font-extrabold shadow-[0_0_20px_rgba(16,185,129,0.75)]",
    },
    {
      ...
      active:
        "bg-cyan-600 text-white border-cyan-300 font-extrabold shadow-[0_0_20px_rgba(6,182,212,0.75)]",
    },
    {
      ...
      active:
        "bg-amber-600 text-white border-amber-300 font-extrabold shadow-[0_0_20px_rgba(245,158,11,0.75)]",
    },
    {
      ...
      active:
        "bg-rose-600 text-white border-rose-300 font-extrabold shadow-[0_0_20px_rgba(244,63,94,0.75)]",
    },
    {
      ...
      active:
        "bg-indigo-600 text-white border-indigo-300 font-extrabold shadow-[0_0_20px_rgba(99,102,241,0.75)]",
    },
  ];
  ...
  for (let r = 1; r < limitRows; r++) {
    ...
    const displayDate =
      rawDate.toLowerCase().includes("result") || rawDate.length > 15
    ...
    for (let c = 1; c < row.length; c++) {
      ...
      if (isUnreleased) {
        ...
        (window as any).vpbDraftPredictions =
          (window as any).vpbDraftPredictions || {};
        ...
        for (let d = 0; d < 4; d++) {
          ...
          const digitLabel =
            d === 0 ? "As" : d === 1 ? "Kop" : d === 2 ? "Kepala" : "Ekor";
          const cellId = `studio_c_${r}_${c}_${d}`;

          const isActive =
            (window as any).vpbStudioActiveNode &&
            (window as any).vpbStudioActiveNode.id === cellId;
          const isChained = (window as any).vpbStudioChains.some(
            (x: any) => x.id === cellId,
          );
          ...
          if (isActive) {
            ringClass =
              "ring-[5px] ring-purple-500 ring-offset-2 ring-offset-slate-900 scale-110 z-20 shadow-[0_0_20px_rgba(168,85,247,0.85)] border-purple-350";
          } else if (isChained) {
            ringClass =
              "ring-[4px] ring-emerald-400 ring-offset-1 ring-offset-slate-900 scale-[1.03] z-10 shadow-[0_0_15px_rgba(52,211,153,0.75)] border-emerald-350";
          }
        ...
        for (let d = 0; d < 4; d++) {
          ...
          if (isActive) {
            ringClass =
              "ring-[5px] ring-purple-500 ring-offset-2 ring-offset-slate-900 scale-110 z-20 shadow-[0_0_20px_rgba(168,85,247,0.85)] animate-pulse border-purple-350";
          } else if (isChained) {
            ringClass =
              "ring-[4px] ring-emerald-400 ring-offset-1 ring-offset-slate-900 scale-[1.03] z-10 shadow-[0_0_15px_rgba(52,211,153,0.75)] border-emerald-350";
          }
...
(window as any).setVpbDraftPrediction = (
  rowIdx: number,
  colIdx: number,
  digitIdx: number,
  val: string,
    ...
    const getDigitStyleInternal = (char: string) => {
      switch (char) {
        ...
        case "9":
          return "bg-gradient-to-br from-yellow-300 via-yellow-105 to-white text-slate-950 border-yellow-400 font-extrabold shadow-sm";
        default:
          return "bg-slate-900 border border-slate-800 text-slate-400 font-bold";
      }
    };
...
(window as any).drawVpbAlurLines = () => {
  ...
  if (points.length >= 2) {
    for (let i = 0; i < points.length - 1; i++) {
      ...
      line.setAttribute(
        "style",
        "filter: url(#vpb-glow); stroke-dasharray: 8 3; animation: vpbDashAnimation 30s linear infinite;",
      );
  ...
  if (activeNode) {
    ...
    if (activeBtn) {
      ...
      if (points.length > 0) {
        ...
        line.setAttribute(
          "style",
          "filter: url(#vpb-glow-dashed); stroke-dasharray: 4 4;",
        );
...
(window as any).onVpbStudioCellClick = (
  id: string,
  rowIdx: number,
  colIdx: number,
  digitIdx: number,
  ...
  if ((window as any).isPickingCell) {
    ...
    const relativePart = dateLabel.includes("(")
      ? dateLabel.split("(")[1].replace(")", "")
      : "H0";
    const positionText =
      digitIdx === -1
        ? digitLabel.toUpperCase()
        : `${colHeader}_${digitLabel.toUpperCase()}`;
...
(window as any).insertVpbOperator = (op: string) => {
  ...
  input.value =
    originalText.substring(0, startPos) + op + originalText.substring(endPos);
...
(window as any).clearVpbFormula = () => {
  ...
  const outText = document.getElementById(
    "vpb-studio-output-text",
  ) as HTMLTextAreaElement;
...
(window as any).generateVpbSpreadsheetPrompt = () => {
  ...
  switch (targetVar) {
    ...
    case "AM_AI":
      ...
      break;
    case "4D_3D_2D":
      targetLabel = "mencari draf tembusan 4D 3D 2D akurat";
      break;
    case "JITU_2D":
      targetLabel = "merumuskan Angka Jitu 2D";
      break;
    case "SHIO":
      targetLabel = "memetakan lintasan line Shio pasaran";
      break;
  }
...
(window as any).applyVpbStudioSpreadsheetFormula = () => {
  ...
  (window as any).closeVpbStudioModal();
  (window as any).showToast(
    "Rumus Spreadsheet sukses digabungkan ke area utama!",
  );
};
...
(window as any).addGlobalRule = async (e: any) => {
  ...
  const text = (
    document.getElementById("input-global-text") as HTMLTextAreaElement
  ).value;
  ...
  const allCb = document.getElementById("global-pool-all") as HTMLInputElement;
  let selectedPools: string[] = [];
  if (allCb && allCb.checked) {
    selectedPools = ["all"];
  } else {
  ...
  try {
    await setDoc(
      doc(db, "globals", id),
      { id, text, poolIds: selectedPools },
      { merge: true },
    );
    (window as any).showToast(
      (window as any).editingGlobalRuleId
        ? "Aturan Global Diperbarui!"
        : "Aturan Global Baru ditambahkan!",
    );
    (window as any).cancelEditGlobalRule();
  } catch (err: any) {
    (window as any).showToast(
      "Gagal menyimpan aturan global: " + err.message,
      true,
    );
  } finally {
    if (btn) {
...
(window as any).editGlobalRule = (id: string) => {
  ...
  (document.getElementById("input-global-text") as HTMLTextAreaElement).value =
    g.text || "";

  (window as any).populateGlobalPoolsChecklist(g.poolIds || ["all"]);
...
(window as any).openAddNewGlobalRule = () => {
  ...
  (window as any).populateGlobalPoolsChecklist(["all"]);
...
(window as any).duplicateGlobalRule = async (id: string) => {
  ...
  const newId = Date.now().toString();
  try {
    await setDoc(doc(db, "globals", newId), {
      id: newId,
      text: g.text + " (Copy)",
    });
...
(window as any).deleteGlobalRule = (id: string) => {
  (window as any).showConfirm(
    "Yakin menghapus aturan global ini secara permanen?",
    async () => {
      if (!(window as any).cloudUserId) return;
      try {
...
(window as any).duplicateMimpi = async (id: string) => {
  ...
  const newId = Date.now().toString();
  try {
    await setDoc(doc(db, "mimpi", newId), {
      id: newId,
      type: m.type,
      no: m.no,
      desc: m.desc + " (Copy)",
...
(window as any).openAddNewPasaran = () => {
  ...
  const autofillEl = document.getElementById(
    "input-pasaran-autofill",
  ) as HTMLInputElement;
  if (autofillEl) autofillEl.checked = false;
  const visibleEl = document.getElementById(
    "input-visible",
  ) as HTMLInputElement;
...
(window as any).addMimpi = async (e: any) => {
  ...
  const type = (
    document.getElementById("input-mimpi-type") as HTMLSelectElement
  ).value;
  ...
  try {
    ...
    (window as any).showToast(
      (window as any).editingMimpiId
        ? "Tafsir mimpi diperbarui!"
        : "Tafsir mimpi baru ditambahkan!",
    );
    (window as any).cancelEditMimpi();
  } catch (err: any) {
...
(window as any).editExtraSource = (id: string) => {
  ...
  document.getElementById("title-form-sandingan")!.innerHTML =
    `<i class="ph-bold ph-pencil"></i> Edit Koneksi Sandingan`;

  (document.getElementById("sand-title") as HTMLInputElement).value =
    s.title || "";
  (document.getElementById("sand-url") as HTMLInputElement).value = s.url || "";
  (document.getElementById("sand-sheet") as HTMLInputElement).value =
    s.sheetName || "";
  (document.getElementById("sand-range") as HTMLInputElement).value =
    s.range || "";
  (document.getElementById("sand-query") as HTMLInputElement).value =
    s.query || "";
  (document.getElementById("sand-prompt") as HTMLInputElement).value =
    s.prompt || "";

  (window as any).populateSandPoolsChecklist(s.poolIds || ["all"]);

  (window as any).openEditModal("form-sandingan", "Edit Koneksi Sandingan");
};

(window as any).toggleExtraSource = async (
  id: string,
...
(window as any).deleteExtraSource = (id: string) => {
  (window as any).showConfirm("Hapus Sumber Data ini?", async () => {
    if (!(window as any).cloudUserId) return;
    await deleteDoc(getDocRef("extra_sources", id));
    (window as any).showToast("Dihapus!", true);
  });
};
...
(window as any).executeImportPermanen = async (e: any) => {
  ...
  const targetType = (document.getElementById("imp-type") as HTMLSelectElement)
    .value;
  const url = (document.getElementById("imp-url") as HTMLInputElement).value;
  const sheetName = (document.getElementById("imp-sheet") as HTMLInputElement)
    .value;
  const range = (document.getElementById("imp-range") as HTMLInputElement)
    .value;
  ...
  if (!sheetId)
    return (window as any).showToast(
      "URL Google Sheets tidak valid. Harus mengandung /d/.../",
      true,
    );
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (targetType === "prompt") {
        const title = getPart(
          (document.getElementById("map-prompt-title") as HTMLInputElement)
            .value,
          0,
        );
        const text = getPart(
          (document.getElementById("map-prompt-text") as HTMLInputElement)
            .value,
          1,
        );
        if (
        ...
        const text = getPart(
          (document.getElementById("map-global-text") as HTMLInputElement)
            .value,
          0,
        );
        if (
          text &&
          text.toLowerCase() !== "aturan" &&
          text.toLowerCase() !== "rule"
        ) {
          const id = Date.now().toString() + count;
        ...
        const type = getPart(
          (document.getElementById("map-mimpi-type") as HTMLInputElement).value,
          0,
        );
        const no = getPart(
          (document.getElementById("map-mimpi-no") as HTMLInputElement).value,
          1,
        );
        const desc = getPart(
    ...
    if (count > 0) {
      await batch.commit();
      (window as any).showToast(
        `SEMPURNA! ${count} data berhasil dimasukkan secara permanen ke Database.`,
      );
      e.target.reset();
    } else {
      (window as any).showToast(
        `Gagal: Data kosong atau pemetaan kolom salah.`,
        true,
      );
    }
  } catch (error) {
    (window as any).showToast(
      "Gagal menyedot. Pastikan URL benar & Sheet Publik.",
      true,
    );
  } finally {
    btn.disabled = false;
...
(window as any).previewFormSheet = async (mode: string, explicitId = null) => {
  ...
  if (explicitId) {
    const s = (window as any).extraSources.find(
      (x: any) => x.id === explicitId,
    );
    if (!s) return;
    url = s.url;
    ...
    if (mode === "sandingan") {
      ...
      sheetName = (document.getElementById("sand-sheet") as HTMLInputElement)
        .value;
      ...
      sheetName = (document.getElementById("imp-sheet") as HTMLInputElement)
        .value;
  ...
  if (!sheetId || !sheetName)
    return (window as any).showToast(
      "Harap isi URL Spreadsheet dan Nama Sheet terlebih dahulu!",
      true,
    );
  ...
  try {
    ...
    const rows = csvData.split("\n");
    let tableHtml =
      '<div class="w-full overflow-x-auto custom-scrollbar"><table class="w-full text-left border-collapse min-w-max"><thead class="bg-slate-800 text-emerald-400 sticky top-0 shadow-sm"><tr>';
    ...
    if (rows.length > 0 && rows[0].trim()) {
      ...
      for (let i = 0; i < maxRows; i++) {
        ...
        if (i === 0) {
          parts.forEach(
            (p, idx) =>
              (tableHtml += `<th class="p-3 md:p-4 border-b-2 border-emerald-500/30 whitespace-nowrap"><span class="text-[9px] md:text-xs text-slate-500 block">Kolom ${String.fromCharCode(65 + idx)}</span>${p}</th>`),
          );
          tableHtml +=
          ...
          parts.forEach(
            (p) =>
              (tableHtml += `<td class="p-3 md:p-4 border-r border-slate-800/30 whitespace-nowrap text-slate-300 font-medium text-sm md:text-base">${p}</td>`),
          );
          tableHtml += "</tr>";
        }
      }
      ...
      if (rows.length > 100)
        tableHtml += `<p class="text-center text-amber-500 mt-6 font-bold bg-amber-500/10 p-3 rounded-lg border border-amber-500/20 max-w-sm mx-auto">⚠️ Menampilkan 100 baris pertama.</p>`;
      body.innerHTML = tableHtml;
    } else {
      body.innerHTML =
        '<div class="text-center mt-10 md:mt-20 text-slate-500"><i class="ph-fill ph-empty text-5xl md:text-6xl mb-3 opacity-50"></i><p class="text-base">Data kosong.</p></div>';
    }
  } catch (e: any) {
...
(window as any).previewTableScrape = async (poolId: string) => {
  ...
  (document.getElementById("pool-selector") as HTMLSelectElement).value =
    poolId;
  const modal = document.getElementById("table-preview-modal");
  const body = document.getElementById("table-preview-body");
  const title = document.getElementById("table-preview-title");
  ...
  try {
    ...
    if (
      (window as any).currentTableData &&
      (window as any).currentTableData.length > 0
    ) {
      let tableHtml =
        '<div class="w-full overflow-x-auto custom-scrollbar"><table class="w-full text-left border-collapse min-w-max"><thead class="bg-slate-800 text-emerald-400 sticky top-0 shadow-sm"><tr>';
      ...
      (window as any).currentTableData.forEach(
        (rowArray: any, index: number) => {
          if (index === 0) {
            rowArray.forEach(
              (cellData: any) =>
                (tableHtml += `<th class="p-4 border-b-2 border-emerald-500/30 whitespace-nowrap">${cellData}</th>`),
            );
            tableHtml +=
              '</tr></thead><tbody class="divide-y divide-slate-800/50">';
          } else {
            ...
            rowArray.forEach(
              (cellData: any) =>
                (tableHtml += `<td class="p-4 border-r border-slate-800/30 whitespace-nowrap text-slate-300 font-medium">${cellData}</td>`),
            );
            tableHtml += "</tr>";
          }
        },
      ...
      body.innerHTML =
        '<div class="text-center mt-20 text-slate-500"><i class="ph-fill ph-empty text-6xl mb-3 opacity-50"></i><p>Tabel kosong atau mapping salah.</p></div>';
    }
  } catch (e: any) {
...
(window as any).previewLiveScrape = async (
  poolId: string,
  drawIdx: number = 0,
) => {
  const p = (window as any).pools.find((x: any) => x.id === poolId);
  ...
  if (p.liveDraws && p.liveDraws.length > drawIdx)
    drawObj = p.liveDraws[drawIdx];
  ...
  try {
    ...
    if (sId && drawObj.sheetNameLive) {
      rawContent = await fetchSpreadsheetTab(
        sId,
        drawObj.sheetNameLive,
        "",
        drawObj.rangeLive,
      );
...
(window as any).parseMarkdown = (text: string): string => {
  ...
  html = html.replace(
    /^###\s+(.+)$/gm,
    '<h3 class="text-sm font-extrabold text-purple-300 mt-4 mb-2 flex items-center gap-2 border-l-2 border-purple-500/50 pl-2"><i class="ph-bold ph-sketch-logo"></i> $1</h3>',
  );
  html = html.replace(
    /^##\s+(.+)$/gm,
    '<h2 class="text-base font-black text-emerald-400 mt-5 mb-2.5 border-b border-slate-800 pb-1.5 flex items-center gap-2"><i class="ph-bold ph-arrow-circle-right animate-pulse text-emerald-500"></i> $1</h2>',
  );
  html = html.replace(
    /^#\s+(.+)$/gm,
    '<h1 class="text-lg font-black text-white mt-6 mb-3 uppercase tracking-wider flex items-center gap-2"><i class="ph-fill ph-crown text-amber-400 animate-bounce"></i> $1</h1>',
  );

  html = html.replace(
    /\*\*(.*?)\*\*/g,
    '<strong class="font-extrabold text-emerald-300">$1</strong>',
  );
  html = html.replace(
    /\*(.*?)\*/g,
  ...
  html = html.replace(
    /```([\s\S]*?)```/g,
    '<pre class="bg-slate-950 p-3 rounded-xl font-mono text-xs text-indigo-300 border border-slate-800 my-3.5 overflow-x-auto shadow-inner">$1</pre>',
  );
  html = html.replace(
    /`([^`]+)`/g,
    '<code class="bg-slate-950 px-1.5 py-0.5 rounded font-mono text-xs text-pink-400 border border-slate-800">$1</code>',
  );

  html = html.replace(
    /^>\s+(.+)$/gm,
    '<blockquote class="border-l-4 border-purple-500 bg-purple-500/10 px-4 py-2.5 my-3.5 rounded-r-xl text-slate-300 italic">$1</blockquote>',
  );
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("|") && line.endsWith("|")) {
      if (!insideTable) {
        ...
        tableStartIndex = i;
        tableHtml +=
          '<div class="overflow-x-auto my-4 rounded-xl border border-slate-800/80 shadow-xl max-w-full"><table class="w-full text-xs text-left text-slate-300 overflow-hidden">';
      }
      ...
      const isHeader =
        tableHtml.includes("</thead>") === false &&
        !tableHtml.includes("<tbody>");
      if (isHeader) {
        tableHtml +=
          '<thead class="bg-slate-900 border-b border-slate-800 text-purple-400 font-extrabold uppercase tracking-widest text-[10px]"><tr>';
        cells.forEach((c) => {
        ...
        tableHtml +=
          '<tr class="border-b border-slate-900/60 hover:bg-slate-900/30 transition-all font-mono">';
        cells.forEach((c) => {
  ...
  html = lines
    .map((l) => {
      if (
        l.trim().startsWith("<div") ||
        l.trim().startsWith("<thead") ||
        l.trim().startsWith("<tr") ||
        l.trim().startsWith("</table") ||
        l.trim().startsWith("<h") ||
        l.trim().startsWith("<li") ||
        l.trim().startsWith("<blockquote") ||
        l.trim().startsWith("<pre")
      ) {
        return l;
      }
...
(window as any).sendFollowUpQuestion = async () => {
  ...
  if (!question) {
    return (window as any).showToast(
      "Ketik pertanyaan anda terlebih dahulu!",
      true,
    );
  }
  ...
  if (sendIcon) {
    sendIcon.className =
      "ph-bold ph-spinner-gap animate-spin text-base text-purple-200";
  }

  // Append user query bubble
  const userBox = document.createElement("div");
  userBox.className =
    "flex items-start gap-2.5 max-w-[85%] ml-auto justify-end animate-slice-in";
  userBox.innerHTML = `
  ...
  if (
    !(window as any).aiChatSessionHistory ||
    (window as any).aiChatSessionHistory.length === 0
  ) {
    const coreBasePrompt =
      (window as any).currentAIBasePrompt ||
      "Kamu adalah Engine SupremeTOTO Master.";
    const coreLastResText =
      (window as any).lastAITextSyair || "Analisis angka jitu.";
    (window as any).aiChatSessionHistory = [
  ...
  try {
    ...
    try {
      data = await fetchGeminiWithRetry(
        apiUrl,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            contents: (window as any).aiChatSessionHistory,
          }),
        },
        2,
      );
      ...
      console.warn(
        "Primary model gemini-3-flash-preview failed, attempting fallback to gemini-3.5-flash...",
        firstErr,
      );
    ...
    if (!replyText || replyText.trim() === "") {
      throw new Error(
        "Gemini mengembalikan respon kosong atau diblokir oleh filter keamanan.",
      );
    }
    ...
    if (sendIcon)
      sendIcon.className = "ph-bold ph-paper-plane-right text-base text-white";
    // Also ensure key listener is kept
    setTimeout(() => {
      const freshInput = document.getElementById(
        "ai-chat-input",
      ) as HTMLTextAreaElement;
      if (freshInput) {
        freshInput.focus();
...
(window as any).generateAI = async () => {
  ...
  if (!sel || !sel.value || !aiRes)
    return (window as any).showToast("Pilih pasaran dulu!", true);

  const apiKey = (window as any).getGeminiAPIKey();

  if (!apiKey || apiKey.trim() === "") {
    return (window as any).showToast(
      "Kunci API Master Belum Diatur! Masukkan API Key Gemini di Panel Admin.",
      true,
    );
  }

  const pPromptSelector = document.getElementById(
    "ai-prompt-selector",
  ) as HTMLSelectElement;
  const pPrompt = pPromptSelector ? pPromptSelector.value : "";

  const manualPromptEl = (document.getElementById("analisis-prompt") ||
    document.getElementById("ai-manual-prompt")) as HTMLTextAreaElement;
  const manualPrompt = manualPromptEl ? manualPromptEl.value : "";

  const btn = (document.getElementById("btn-start-ai") ||
    document.getElementById("btn-generate-ai")) as HTMLButtonElement;
  if (btn) {
  ...
  const activeGlobals = ((window as any).globalRules || []).filter((g: any) => {
    if (!g.poolIds || g.poolIds.length === 0 || g.poolIds.includes("all"))
      return true;
    return g.poolIds.includes(sel.value);
  });

  if (activeGlobals.length > 0) {
    basePrompt += "\n[ATURAN SYSTEM INTI]:\n";
    activeGlobals.forEach((g: any) => (basePrompt += `- ${g.text}\n`));
  }
...
`;
  ...
  const activeSources = ((window as any).extraSources || []).filter(
    (s: any) => {
      ...
      if (!s.poolIds || s.poolIds.length === 0 || s.poolIds.includes("all"))
        return true;
      return s.poolIds.includes(sel.value);
    },
  );
  if (activeSources.length > 0) {
    (window as any).showToast(
      `Menarik ${activeSources.length} sumber data sandingan tambahan...`,
    );
    try {
      for (let s of activeSources) {
        if (s.sheetId && s.sheetName) {
      ...
      (window as any).showToast(
        "Peringatan: Gagal menarik beberapa data sandingan.",
        true,
      );
    }
  }
  ...
  try {
    ...
    if (infoEl)
      infoEl.innerHTML = `<span class="bg-purple-600 text-white px-2 py-1 rounded text-xs font-bold border border-purple-400">GEMINI 3.0</span> <span class="text-xs text-slate-400 ml-2">Analisis Selesai</span>`;
    ...
    if (typeof (window as any).updateGeminiStatusIndicator === "function") {
      (window as any).updateGeminiStatusIndicator(
        "failed",
        error.message || "Koneksi AI Gagal",
      );
    }
    ...
    aiRes.innerHTML = `
        ...
        `;
    if (infoEl)
      infoEl.innerHTML = `<span class="text-red-400 text-xs font-bold leading-normal"><i class="ph-fill ph-warning-circle"></i> Koneksi AI Gagal</span>`;
  } finally {
    if (btn) {
...
(window as any).renderSyairVarsAdmin = () => {
  const list =
    document.getElementById("admin-syair-variables-list") ||
    document.getElementById("syair-vars-list");
  if (!list) return;
...
(window as any).updateAdminSyairPreview = () => {
  ...
  const bgUrl =
    (document.getElementById("edit-syair-bg") as HTMLInputElement)?.value ||
    t.bgUrl ||
    "";
  const mainCol =
    (document.getElementById("edit-syair-color-main") as HTMLInputElement)
      ?.value ||
    t.mainColor ||
    "#10b981";
  const accCol =
    (document.getElementById("edit-syair-color-accent") as HTMLInputElement)
      ?.value ||
    t.accentColor ||
    "#3b82f6";
  const htmlVal =
    (document.getElementById("input-syair-html") as HTMLTextAreaElement)
      ?.value ||
    t.html ||
    (window as any).defaultSyairHTML;
  const footerVal =
    (document.getElementById("edit-syair-footer") as HTMLInputElement)?.value ||
    t.footer ||
    "SUPREME TOTO AI GENERATED";

  const previewContainer =
    document.getElementById("admin-syair-preview") ||
    document.getElementById("admin-syair-preview-canvas");
  if (previewContainer) {
...
(window as any).openSyairEditor = () => {
  ...
  if (!aiText)
    return (window as any).showToast(
      "Analisis AI kosong! Buat analisis dulu.",
      true,
    );
  ...
  const pName =
    pSel && pSel.selectedIndex >= 0
      ? pSel.options[pSel.selectedIndex].text
      : "PASARAN";

  let htmlContent =
    (window as any).syairTemplate?.html || (window as any).defaultSyairHTML;
  ...
  htmlContent = htmlContent
    .replace(
      /{{MAIN_COLOR}}/g,
      (window as any).syairTemplate?.mainColor || "#10b981",
    )
    .replace(
      /{{ACCENT_COLOR}}/g,
      (window as any).syairTemplate?.accentColor || "#3b82f6",
    )
    .replace(
      /{{FOOTER_TEXT}}/g,
      (window as any).syairTemplate?.footer || "SUPREME TOTO AI",
    );

  const canvasObj = document.getElementById("capture-canvas");
  if (canvasObj) {
    canvasObj.innerHTML = htmlContent;
    canvasObj.style.backgroundImage = (window as any).syairTemplate?.bgUrl
      ? `url('${(window as any).syairTemplate.bgUrl}')`
      : "";
...
(window as any).downloadSyairPNG = async () => {
  ...
  const convertUrlToBase64 = async (url: string): Promise<string> => {
    ...
    try {
      ...
      console.warn(
        "Failed to proxy/convert image to base64, using fallback URL:",
        url,
        e,
      );
      return url;
...
(window as any).downloadHasilPDF = async () => {
  ...
  overlay.className =
    "fixed inset-0 bg-slate-900/90 z-[9999] flex flex-col items-center justify-center";
...
(window as any).resetSyairHTML = async () => {
  ...
  if (
    ...
    try {
      ...
      (window as any).showToast(
        "Desain direst! Semua modifikasi HTML otomatis hilang.",
      );
    } catch (e: any) {
...
(window as any).saveSyairTemplate = async (e: any) => {
  e.preventDefault();
  if (!(window as any).cloudUserId) return;
  const btn = (document.getElementById("btn-save-syair") ||
    e.submitter ||
    e.target.querySelector('button[type="submit"]')) as HTMLButtonElement;
  ...
  try {
    const docData = {
      bgUrl:
        (document.getElementById("edit-syair-bg") as HTMLInputElement)?.value ||
        (window as any).syairTemplate?.bgUrl ||
        "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2564&auto=format&fit=crop",
      opacity:
        (document.getElementById("edit-syair-opacity") as HTMLInputElement)
          ?.value ||
        (window as any).syairTemplate?.opacity ||
        "80",
      mainColor:
        (document.getElementById("edit-syair-color-main") as HTMLInputElement)
          ?.value ||
        (window as any).syairTemplate?.mainColor ||
        "#fbbf24",
      accentColor:
        (document.getElementById("edit-syair-color-accent") as HTMLInputElement)
          ?.value ||
        (window as any).syairTemplate?.accentColor ||
        "#b45309",
      footer:
        (document.getElementById("edit-syair-footer") as HTMLInputElement)
          ?.value ||
        (window as any).syairTemplate?.footer ||
    ...
    await setDoc(doc(db, "settings", "syair_template"), docData, {
      merge: true,
    });
    ...
    try {
      ...
      if (file.name.endsWith(".json") || content.trim().startsWith("{")) {
        try {
          ...
          if (!html) {
            (window as any).showToast(
              "File JSON tidak valid / tidak ada data HTML syair",
              true,
            );
            return;
          }
          ...
          const bgUrl =
            ...
            "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2564&auto=format&fit=crop";
          const opacity =
            parsed.opacity || (window as any).syairTemplate?.opacity || "80";
          const mainColor =
            parsed.mainColor ||
            (window as any).syairTemplate?.mainColor ||
            "#fbbf24";
          ...
          await setDoc(
            doc(db, "settings", "syair_template"),
            {
              html,
              variables: (window as any).syairVariables,
              bgUrl,
          ...
          (window as any).showToast(
            "Templat Syair & Variabel berhasil diimpor!",
          );
          return;
        } catch (jsonErr) {
      ...
      const accentColor =
        (window as any).syairTemplate?.accentColor || "#b45309";
      const footer =
        (window as any).syairTemplate?.footer || "Utamakan Prediksi Sendiri";
...
(window as any).downloadPrompts = () => {
  if (!(window as any).aiPrompts || (window as any).aiPrompts.length === 0)
    return (window as any).showToast(
      "Tidak ada data pola untuk dibackup.",
      true,
    );
  const dataStr =
    "data:text/json;charset=utf-8," +
    encodeURIComponent(JSON.stringify((window as any).aiPrompts, null, 2));
  const a = document.createElement("a");
  a.href = dataStr;
...
(window as any).downloadGlobals = () => {
  if (!(window as any).globalRules || (window as any).globalRules.length === 0)
    return (window as any).showToast(
      "Tidak ada data aturan untuk dibackup.",
      true,
    );
  const dataStr =
    "data:text/json;charset=utf-8," +
    encodeURIComponent(JSON.stringify((window as any).globalRules, null, 2));
  const a = document.createElement("a");
  a.href = dataStr;
...
(window as any).autoSeedDatabase = async () => {
  ...
  try {
    ...
    if (conf) b.set(doc(db, "settings", "config"), conf, { merge: true });
    if (sandConf)
      b.set(doc(db, "settings", "sandingan_config"), sandConf, { merge: true });
    if (syConf)
      b.set(doc(db, "settings", "syair_template"), syConf, { merge: true });
...
(window as any).switchToLocalMode = (reason: string) => {
  ...
  if (statusText) {
    statusText.className =
      "text-[8px] text-amber-400 font-extrabold tracking-wider leading-none truncate";
    statusText.innerHTML = `DB: LOKAL`;
  }
  const statusIcon = document.getElementById("cloud-status-icon");
  if (statusIcon) {
    statusIcon.className =
      "ph-fill ph-circle text-[6px] text-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.6)] animate-pulse";
  }
  ...
  (window as any).dataBukuMimpi = loadLocal(
    "superemetoto_mimpi",
    parsedBackup.mimpi || [],
  );
  (window as any).aiPrompts = loadLocal(
    "superemetoto_prompts",
    parsedBackup.prompts || [],
  );
  (window as any).globalRules = loadLocal(
    "superemetoto_globals",
    parsedBackup.globals || [],
  );
  (window as any).extraSources = loadLocal(
    "superemetoto_extra_sources",
    parsedBackup.extraSources || [],
  );
  ...
  try {
    ...
    if (localConf) {
      ...
      (window as any).appConfig = parsedBackup.config ||
        parsedBackup.settings?.config || {
          apiKey: (window as any).DEFAULT_API_KEY,
        };
    }
  } catch {
  ...
  try {
    const localSand = localStorage.getItem(
      "superemetoto_settings_sandingan_config",
    );
    if (localSand) {
      (window as any).sandinganConfig = JSON.parse(localSand);
    } else {
      (window as any).sandinganConfig =
        parsedBackup.sandinganConfig ||
        parsedBackup.settings?.sandinganConfig ||
        {};
    }
  } catch {
    (window as any).sandinganConfig = {};
  ...
  const glbSandInput = document.getElementById(
    "global-sand-prompt",
  ) as HTMLTextAreaElement;
  ...
  // Load Syair Template
  try {
    const localSyair = localStorage.getItem(
      "superemetoto_settings_syair_template",
    );
    if (localSyair) {
      (window as any).syairTemplate = JSON.parse(localSyair);
    } else {
      (window as any).syairTemplate =
        parsedBackup.syairTemplate ||
        parsedBackup.settings?.syairTemplate ||
        {};
    }
  } catch {
    (window as any).syairTemplate = {};
  ...
  const t = (window as any).syairTemplate;
  (window as any).syairVariables = t.variables || [
    "PASARAN",
    "TANGGAL",
    "SHIO",
    "BBFS",
  ...
  const btnOp = document.getElementById(
    "edit-syair-opacity",
  ) as HTMLInputElement;
  ...
  const btnM = document.getElementById(
    "edit-syair-color-main",
  ) as HTMLInputElement;
  if (btnM) btnM.value = t.mainColor || "#10b981";

  const btnA = document.getElementById(
    "edit-syair-color-accent",
  ) as HTMLInputElement;
  ...
  const btnH = document.getElementById(
    "input-syair-html",
  ) as HTMLTextAreaElement;
  ...
  if (auth) {
    signInAnonymously(auth)
      .then(() => {
        onAuthStateChanged(auth, (user) => {
          if (user) {
            ...
            if (statusText) {
              statusText.className =
                "text-[8px] text-emerald-500 font-extrabold tracking-wider leading-none truncate";
              statusText.innerHTML = `DB: AKTIF`;
            }
            const statusIcon = document.getElementById("cloud-status-icon");
            if (statusIcon) {
              statusIcon.className =
                "ph-fill ph-circle text-[6px] text-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]";
            }
            ...
            document
              .getElementById("status-db-pribadi")
            ...
            onSnapshot(collection(db, "pools"), (snap) => {
              ...
              localStorage.setItem(
                "superemetoto_pools",
                JSON.stringify((window as any).pools),
              );
              (window as any).renderAdminLists();
              (window as any).renderDropdowns();
              ...
              if (isFirstPoolsLoad) {
                ...
                if (snap.empty) {
                  console.log(
                    "Database pools are empty on first Firestore load, triggering automatic auto seed...",
                  );
            ...
            onSnapshot(collection(db, "mimpi"), (snap) => {
              ...
              localStorage.setItem(
                "superemetoto_mimpi",
                JSON.stringify((window as any).dataBukuMimpi),
              );
              (window as any).renderAdminLists();
              (window as any).renderBukuMimpi();
            });
            ...
            onSnapshot(collection(db, "prompts"), (snap) => {
              ...
              localStorage.setItem(
                "superemetoto_prompts",
                JSON.stringify((window as any).aiPrompts),
              );
              (window as any).renderAdminLists();
            });
            ...
            onSnapshot(collection(db, "globals"), (snap) => {
              ...
              localStorage.setItem(
                "superemetoto_globals",
                JSON.stringify((window as any).globalRules),
              );
              (window as any).renderAdminLists();
            });
            ...
            onSnapshot(collection(db, "extra_sources"), (snap) => {
              ...
              localStorage.setItem(
                "superemetoto_extra_sources",
                JSON.stringify((window as any).extraSources),
              );
              (window as any).renderAdminLists();
              (window as any).updateAIInfoPanel();
            });
            ...
            onSnapshot(doc(db, "settings", "config"), (docSnap) => {
              if (docSnap.exists()) {
                ...
                localStorage.setItem(
                  "superemetoto_settings_config",
                  JSON.stringify((window as any).appConfig),
                );
                const ak = document.getElementById(
                  "input-setting-apikey",
                ) as HTMLInputElement;
            ...
            onSnapshot(doc(db, "settings", "sandingan_config"), (docSnap) => {
              if (docSnap.exists()) {
                ...
                localStorage.setItem(
                  "superemetoto_settings_sandingan_config",
                  JSON.stringify((window as any).sandinganConfig),
                );
                const glbSandInput = document.getElementById(
                  "global-sand-prompt",
                ) as HTMLTextAreaElement;
            ...
            onSnapshot(doc(db, "settings", "syair_template"), (docSnap) => {
              if (docSnap.exists()) {
                ...
                localStorage.setItem(
                  "superemetoto_settings_syair_template",
                  JSON.stringify((window as any).syairTemplate),
                );
                (window as any).syairVariables = t.variables || [
                  "PASARAN",
                  "TANGGAL",
                ...
                const btnBg = document.getElementById(
                  "edit-syair-bg",
                ) as HTMLInputElement;
                if (btnBg) btnBg.value = t.bgUrl || "";
                ...
                if (btnH)
                  btnH.value = t.html || (window as any).defaultSyairHTML;
      ...
      .catch((err) => {
        ...
        (window as any).switchToLocalMode(
          err.message || "Firebase Auth failed",
        );
      });
  } else {
...
(window as any).tafsirMimpiAI = async () => {
  ...
  if (!story || story.trim() === "")
    return (window as any).showToast("Mimpi masih kosong!", true);

  const apiKey = (window as any).getGeminiAPIKey();
  if (!apiKey || apiKey.trim() === "")
    return (window as any).showToast("Kunci API Master Belum Diatur!", true);
  ...
  try {
    ...
    const promptTafsir =
      "Bertindaklah sebagai Pakar Tafsir Mimpi Togel (Erek-erek) profesional.\n" +
...
(window as any).getGeminiAPIKey = (): string => {
  ...
  const activeKeys = keys.filter(
    (k: any) => k.active && k.apiKey && k.apiKey.trim() !== "",
  );
  ...
  if (activeKeys.length > 0) {
    ...
    console.log(
      `[getGeminiAPIKey] Using active Gemini Key: "${randomKey.title}"`,
    );
...
(window as any).renderGeminiKeys = () => {
  ...
  keys.forEach((k: any) => {
    ...
    const activeIcon = k.active
      ? "ph-fill ph-toggle-right text-lg text-emerald-400"
      : "ph-bold ph-toggle-left text-lg text-slate-500";
...
(window as any).saveGeminiKey = async () => {
  ...
  if (!apiKey)
    return (window as any).showToast("Masukkan Kunci API Gemini!", true);
...
(window as any).exportGeminiKeys = () => {
  try {
    ...
    if (keys.length === 0) {
      return (window as any).showToast(
        "Daftar Kunci kosong, tidak ada yang dieksport.",
        true,
      );
    }
    const dataStr =
      "data:text/json;charset=utf-8," +
      encodeURIComponent(JSON.stringify(keys, null, 2));
    const a = document.createElement("a");
    a.href = dataStr;
  ...
  reader.onload = async (ev: any) => {
    try {
      ...
      if (!Array.isArray(parsed)) {
        return (window as any).showToast(
          "Format file salah! Harus berupa list/array JSON Kunci Gemini.",
          true,
        );
      }
      if (
      ...
      parsed.forEach((newK: any) => {
        if (!newK.id)
          newK.id = Date.now() + Math.random().toString(36).substring(4);
        if (!newK.title) newK.title = "Imported Key";
        if (!newK.apiKey) return;
        if (newK.active === undefined) newK.active = true;

        const existsIdx = keys.findIndex(
          (x: any) => x.id === newK.id || x.apiKey === newK.apiKey,
        );
        if (existsIdx !== -1) {
...
(window as any).tidyUpPrompt = async (elementId: string) => {
  ...
  if (!text) {
    return (window as any).showToast(
      "Form masih kosong, tidak ada yang bisa dirapikan!",
      true,
    );
  }
  ...
  el.value =
    "Sedang merapikan kalimat menggunakan AI, mohon tunggu sebentar...";
  el.disabled = true;
  ...
  try {
    ...
    if (
      elementId === "global-sand-prompt" &&
      typeof (window as any).saveGlobalSandinganPrompt === "function"
    ) {
